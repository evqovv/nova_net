...
class tcp_connection : public ::std::enable_shared_from_this<tcp_connection> {
  public:
    ...
    void set_message_handler(new_message_handler_t handler) {
    ...
    void register_write_event() {
        ...
        ::std::weak_ptr<tcp_connection> weak = shared_from_this();
        loop_.update_channel(fd_, EPOLLIN | EPOLLOUT, [weak](uint32_t ev) {
            if (auto self = weak.lock()) [[likely]] {
                if (ev & EPOLLIN) {
                    self->recv();
                }
                if (ev & EPOLLOUT) {
    ...
    void register_read_event() {
        ::std::weak_ptr<tcp_connection> weak = shared_from_this();

        loop_.add_channel(fd_, EPOLLIN, [weak](uint32_t) {
    ...
    void unregister_write_event() {
        ...
        ::std::weak_ptr<tcp_connection> weak = shared_from_this();
        loop_.update_channel(fd_, EPOLLIN, [weak](uint32_t) {
            if (auto self = weak.lock()) {
                self->recv();
            }
        });
...
class tcp_acceptor : public ::std::enable_shared_from_this<tcp_acceptor> {
  public:
    ...
    void start() {
        base::listen(fd_);
        ::std::weak_ptr<tcp_acceptor> weak = shared_from_this();
        loop_.add_channel(fd_, EPOLLIN, [weak](uint32_t events) {
            if (auto self = weak.lock()) [[likely]] {
                self->on_readable();
            }
        });
    ...
    void on_readable() {
        while (true) {
            ...
            if (new_fd != invalid_socket) [[likely]] {
                if (handler_) [[likely]] {
                    auto conn = std::make_shared<tcp_connection>(loop_, new_fd);
                    conn->register_read_event();
                    handler_(conn);
                } else [[unlikely]] {
